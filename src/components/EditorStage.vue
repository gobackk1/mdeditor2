<template>
	<div class="editor">
		...
		<div class="editor__stage stage">
			...
			<button @click="onClickStarButton">
				{{note.isFavorite ? '⭐️' : '☆'}}
			</button>
			<button @click="onClickTrashButton">
				{{note.isTrash ? '●' : '○'}}
			</button>
			<textarea v-if="note" v-model="note.title" class="stage__textarea" name="" id="" cols="30" rows="10"></textarea>
		</div>
	</div>
</template>
